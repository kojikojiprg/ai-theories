...
- **Markdown の強調記法の前後には半角スペースを入れる** 。日本語文中で `**強調**` の直後に日本語文字が続くと、CommonMark のフランキング規則の判定により強調として描画されないエディタ・ビューアがあるため、必ず前後を半角スペースで区切る。
  - 悪い例: `**表の番号順に学習する**ことを推奨します。`
  - 良い例: `**表の番号順に学習する** ことを推奨します。`
  - 行頭・行末や、既に半角スペース・改行が隣接している場合は追加不要。
- 太字( `**` )・斜体( `*` )・インラインコード(`` ` ``)・リンク記法( `[...](...)` )のいずれにも同じ規則を適用する。
- 描画後に前後の半角スペースが空きとして見えるが、 **表示の確実性を優先する** 方針とする。
