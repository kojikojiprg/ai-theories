...
- ノートブック(`.ipynb`)は **実行結果(outputs)を含めた状態で Git 管理する**。GitHub 上のノートブックプレビューでグラフや出力例をそのまま確認できるようにするのが目的。
- ノートブックを実行した状態(セルの出力が反映された状態)でコミットする。コミット前に該当セルを実行し、出力が最新であることを確認する。
- 結果・考察は Markdown セルにも文章として残すこと(出力のグラフ・数値の読み取り結果を本文で言語化する)。
