...
| `src/utils/statistics.py`            | `compute_mean_to_rms_ratio()`、`compute_always_negative_unit_ratio()`、`compute_gradient_norm_by_unit_group()`、`compute_gradient_norm_per_layer()`(004)。005 で`compute_fertility()`・`compute_unknown_rate()`・`compute_chunk_length_statistics()`・`compute_exact_match_rate()`を追加 | 004, 005     |
