...
| `src/data/tokenizer.py`              | `learn_bpe()`・`BPETokenizer`(BPE の学習・符号化・`decode()`、`byte_level`によるバイトレベル BPE 切り替え)、`pretokenize()`(事前分割、`chunk_split_mode`。`whitespace` は空白をチャンク先頭に保持し可逆)、`viterbi_segment()`(Unigram 言語モデルの Viterbi 最尤分割)、`UnigramTokenizer`・`train_unigram_model()`(sentencepiece への語彙学習の委譲)、`try_decode_byte_level_symbol()`(バイトレベル語彙の 1 シンボルが単一文字を表すかを判定) | 005          |
| `src/utils/statistics.py`            | `compute_mean_to_rms_ratio()`、`compute_always_negative_unit_ratio()`、`compute_gradient_norm_by_unit_group()`、`compute_gradient_norm_per_layer()`(004)。005 で`compute_fertility()`・`compute_unknown_rate()`・`compute_chunk_length_statistics()`(先頭の連続空白を除いた統計も返す)・`compute_exact_match_rate()`・`compute_segmentation_agreement_rate()`(トークン境界集合の Jaccard 係数)・`compute_character_coverage()`(語彙による文字被覆率)を追加 | 004, 005     |
