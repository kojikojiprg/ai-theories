...
| `src/utils/visualization.py`         | `plot_attention_heatmap()`、`plot_multi_head_attention()`、`plot_learning_curves()`(001)。004 で`plot_seed_scatter()`(複数シードの散布図)・`plot_bar_by_layer()`(層ごとの棒グラフ)・`plot_function_curves()`(関数曲線の比較)・`plot_learning_curves_multi_seed()`(条件ごとに複数シードの学習曲線を重ね描き)を追加 | 001, 004     |
...
| `src/data/text.py`                   | `CharacterLevelTokenizer`、`load_tiny_shakespeare()`、`split_train_val()`、`get_random_batch()`            | 004          |
| `src/utils/statistics.py`            | `compute_mean_to_rms_ratio()`、`compute_always_negative_unit_ratio()`、`compute_gradient_norm_by_unit_group()`、`compute_gradient_norm_per_layer()` | 004          |
